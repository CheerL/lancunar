...
def prepareDataThread(proc, dataQueue, numpyImages, numpyGT, params):
    ''' the thread worker to prepare the training data'''
    num_image = len(numpyImages)
    num_iter = params['ModelParams']['numIterations']
    batchsize = params['ModelParams']['batchsize']
    # np.random.seed()

    for num in range(proc, batchsize * num_iter, params['ModelParams']['nProc']):
        key = numpyImages.keys()[num % num_image]
        tempimage = numpyImages[key].copy()
        tempGT = numpyGT[key].copy()
        ...
        tempimage = np.rot90(tempimage, randomi) * (np.random.rand(1) + 0.5)
...
if __name__ == '__main__':
    ...
    params['ModelParams']['numIterations'] = 100000  # the number of total training iterations
    params['ModelParams']['baseLR'] = 5e-3  # the learning rate, initial one
    ...
    params['DataManagerParams']['VolSize'] = np.asarray([64, 64, 32], dtype=int) # the size of the crop image
    params['DataManagerParams']['TestStride'] = np.asarray([64, 64, 32], dtype=int) # the stride of the adjacent crop image in testing phase and validation phase
    ...
    if len(train) > 0:
        ...
        for proc in range(0, params['ModelParams']['nProc']):
            dataPreparation[proc] = Process(
                ...
                args=(proc, dataQueue, numpyImages, numpyGT, params)
            )
